...
import { FiCheckCircle, FiImage, FiFile, FiX, FiXCircle, FiTruck, FiDollarSign, FiSearch, FiRefreshCw, FiCalendar, FiAlertTriangle, FiPrinter, FiSettings, FiEdit2, FiSave, FiMessageSquare } from 'react-icons/fi'
...
export default function ClientOrders() {
  ...
  async function handlePrintThermal(order) {
    ...
    doc.setFontSize(14)
    doc.setFont('helvetica', 'bold')
    doc.text(`Pedido #${padId(order.id)}`, w / 2, y, { align: 'center' })
    y += 5
    doc.setFontSize(8)
    doc.setFont('helvetica', 'normal')
    ...
    <div>
      ...
      <div className="flex gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Buscar por nº do pedido, nome ou celular..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter') setAppliedSearch(search) }}
            className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-800 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-pink-500 focus:border-transparent outline-none"
          />
        </div>
        <select
          value={filterStatus}
          onChange={(e) => setFilterStatus(e.target.value)}
          className="px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-800 dark:text-white focus:ring-2 focus:ring-pink-500 focus:border-transparent outline-none"
        >
      ...
      <div className="space-y-4">
        {orders.filter((order) => {
          ...
          return (
            padId(order.id).includes(q) || String(order.id).includes(q) ||
                ...
                <div className="text-right flex flex-col items-end gap-1">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors"
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-lg font-bold text-green-600 dark:text-green-400">{BRL(total)}</p>
                    {reservation && (
                      <p className="text-xs text-gray-500 dark:text-gray-400">Reserva: {BRL(reservation)}</p>
                    )}
                    {order.depositDivergence && (
                      <p className="text-xs text-yellow-600 dark:text-yellow-400 flex items-center justify-end gap-1">
                        <FiAlertTriangle size={10} /> Divergência de valor
                      </p>
                    )}
                    {depositAmount && depositAmount !== reservation && (
                      <p className="text-xs text-blue-600 dark:text-blue-400">Depósito real: {BRL(depositAmount)}</p>
                    )}
                    {balanceToReceive !== null && balanceToReceive > 0 && order.status !== 'CANCELLED' && (
                      <p className="text-xs font-semibold text-orange-600 dark:text-orange-400">Receber na entrega: {BRL(balanceToReceive)}</p>
                    )}
                  </div>
                </div>
              </div>
      ...
      {depositModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-slideDown">
            <div className="flex items-center justify-between px-6 py-4 border-b bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
              <h2 className="text-lg font-semibold text-gray-800 dark:text-white">Verificar Depósito - Pedido #{padId(depositModal.id)}</h2>
      ...
      <ConfirmModal
        ...
        message={statusConfirm ? `Alterar status do pedido para "${statusLabels[statusConfirm.status]}"?` : ''}
        confirmText="Confirmar"
        confirmColor="bg-blue-600 hover:bg-blue-700"
        onConfirm={() => statusConfirm && updateStatus(statusConfirm.orderId, statusConfirm.status)}
      />
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden animate-slideDown">
            ...
            <div className="p-6 flex flex-col items-center text-center space-y-4">
              <p className="text-gray-600 dark:text-gray-300">
                Tem certeza que deseja cancelar o Pedido #{padId(cancelConfirm.id)}?
              </p>
      ...
      {printOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm animate-fadeIn" onClick={() => { URL.revokeObjectURL(printOrder.pdfUrl); setPrintOrder(null) }} />
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-slideDown flex flex-col" style={{ maxHeight: '90vh' }}>
            <div className="flex items-center justify-between px-6 py-4 border-b bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
              <h2 className="text-lg font-semibold text-gray-800 dark:text-white">Pedido #{padId(printOrder.order.id)}</h2>
              <button onClick={() => { URL.revokeObjectURL(printOrder.pdfUrl); setPrintOrder(null) }} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors">
                <FiX size={20} />
              </button>
            </div>
            ...
            <div className="px-6 py-4 border-t bg-gray-50 dark:bg-gray-700 dark:border-gray-600 flex justify-end gap-3">
              <button
                onClick={() => { URL.revokeObjectURL(printOrder.pdfUrl); setPrintOrder(null) }}
                className="px-4 py-2.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors"
              >
                Fechar
              </button>
